...
if ($argCount < 2 || $ARGV[0] eq "-h" || $ARGV[0] eq "--help")
{
	...
	my $newProdcutVersion = createVersion($AssemblyInfoCSPath,$buildNum,$prefix);
	...
	updateVersionInFile($AssemblyInfoCSPath,$newFileVersion, $prefix);
	exit 0;
}
...
sub updateSPOLAppNameInFile
{
	...
	print "INFO: Updating App Name in file $file.\n";
...
sub updateSPOLAppShortNameInFile
{
	...
	print "INFO: Updating App Short Name in file $file.\n";
	print "  Build #      = $buildType\n";
	...
	my	$newAppName = "NextLabsPurgeUserTool";
	my  $newAppTitle = "NextLabs PurgeUser Tool";
	if ($buildType eq "release")
	{
		$newAppName = "NextLabsPurgeUserTool";
		$newAppTitle = "NextLabs PurgeUser Tool";
	}

	if ($buildType eq "dev")
	{
		$newAppName = "NextLabsPurgeUserTool_Dev";
		$newAppTitle = "NextLabs PurgeUser Tool Dev";
	}

	if ($buildType eq "pcv_smdc")
	{
		$newAppName = "NextLabsPurgeUserTool_PCV";
		$newAppTitle = "NextLabs PurgeUser Tool PCV";
	}
...
#		print "LINE: $buf";
		...
		$buf =~ s/NextLabsPurgeUserTool/$newAppName/g; # Modify the name in xml
		$buf =~ s/NextLabs PurgeUser Tool/$newAppTitle/g; # Modify the title in xml
...
sub updateSPOLAppIDInFile
{
	...
	print "INFO: Updating PurgeUser Tool App ID in file $file.\n";
	print "  Build #      = $buildType\n";
